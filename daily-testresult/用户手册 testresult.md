...
[***bug*** 1055]地址栏一删就全部删除了，无法一个个字符删除。导致无法微改路径(*用backspace键删除时有此问题，用delete键删除时无此问题*)

[**bug** 1056]适用菜单不恰当，以前给陈老师的邮件发的word文件里就提过。菜单应考虑环境上下文，举例1：在回收站内文件上右键应是“还原/剪切/删除/属性”，而我们现在的却是“/打开/打开方式/复制/剪切/删除/永久删除/重命名/发送/复制路径/压缩/解压缩/详情”。举例2：非常压缩
文件右键不应出现“解压”菜单项
[feature]个人空间-根目录下也要支持右键及多选等 文件操作{更换图标解决此问题}

[feature]文件管理器开多个窗口，相互间拖动进行文件操作等

[ bug 1090 ] 在右侧或者下方右键选择排序时，弹出的菜单栏超出界面范围，无法点击。
...
[bug] 687 投影菜单显示效果不好看。且投影菜单显示时通知栏不显示，此时在显的太突兀- 2016-11-24 
 ...
 1. 音量设置时静音和有声的图标风格不同意，建议UI方面修改一下。
 
 2. 点击网络图标弹出的页面中，unable按键切换有问题，表现为从able到unable的不能切换，反过来可以。
...
[bug 1058] 网络设置， 最上面的一条，文字顶头， 与前后页效果不符。
...
# 泰捷视频：
[bug 1070]播放视频时，卡在0％（该问题已经整理在github上，不过好像之前一直都有这个问题，有时候能播有时候不能播）
...
# 电源:
［ｂｕｇ 1071］电源->锁定 没有任何响应<没有设置锁屏密码>
...
［ｂｕｇ 820］ 代理模式模块存在，输入数据不能保存且功能未实现。
...
［ｂｕｇ］ 1045 账户-添加账户-点击QQ不弹出相应的界面

2. 设置字号为超大时，应用商店的安装按钮只能显示一半，开始菜单的排序选项、右键菜单以及系统自带浏览器的菜单选项也无法显示完全
3. 壁纸来源中有两个‘壁纸‘选项，且最后一个壁纸选项包含动态壁纸和普通图片，建议分别与其它选项合并
4. 打开壁纸来源菜单，进入第三个选项（用快图打开的那个），之后随便进入一个文件夹，比如screenshots，然后退回到壁纸来源菜单，再次进入选项时会直接进入screenshots文件夹
5. 使用快图选择壁纸，应用壁纸后，实际壁纸的尺寸与之前预览的不一致，实际的要大很多
6. 无法设置电源管理中的节能模式
7. 语言设置的倒数第六个选项无法正常显示
8. 进入关于设备 ->法律信息，之后双击退回按钮，界面异常
9. 打开设置界面后最小化，无法输入字符 ，必须关闭设置界面才可以恢复
10. 无法通过蓝牙接收文件（收到接收文件请求时，openthos界面会弹出请确认是否接收的提示，但是没有弹出选择窗口。如果去通知栏打开窗口的话，窗口中没有同意接收的按钮）
11. 以太网设置中切换到静态ip，保存，再切换到dhcp，保存，反复一两次，有很大概率会弹出 “设置无响应“ 或者是 “很抱歉，系统界面已停止运行“ 的提示
12. 设置中配置以太网静态IP的排版有问题
